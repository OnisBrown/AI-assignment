...
;; )
 ...
 (:action up 
 	:parameters(?r - robot ?y - tile ?x - tile)
    :precondition(and 
    	(not(robot-at ?r ?x))(clear ?x)(not(cleaned ?x)))
    :effect(robot-at ?r ?x)
 )


;; (:action down 
;;    :parameters(?r - robot ?x - tile)
;;    :precondition(and 
;                 (down ?r ?x)(not(robot-at ?r ?x)) (clear ?x) (not(cleaned ?x)))
;;    :effect(robot-at ?r ?x)
 ;;)
...
;;  	:parameters(?r - robot ?x - tile)
;;    :precondition(and 
 ;;                 (left ?r ?x)(not(robot-at ?r ?x)) (clear ?x) (not(cleaned ?x)))
;;    :effect(robot-at ?r ?x)
;; )
